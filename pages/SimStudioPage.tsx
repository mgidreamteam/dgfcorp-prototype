...
import { analyzeUserIntent, getAnswerFromSpec, generateHardwareSpecs } from '../services/gemini';
import { AlertCircle, Cloud, X, CloudDownload, Trash2, Loader2, Map as MapIcon, Globe } from 'lucide-react';
...
const SimStudioPage: React.FC = () => {
  ...
  const { isLoaded } = useJsApiLoader({ id: 'google-map-script', googleMapsApiKey: import.meta.env.VITE_FIREBASE_API_KEY || '' });
  ...
  useEffect(() => {
    if (!hasInitiallyLoaded.current && !projectId && projects.length > 0) {
      ...
      if (mostRecentProject) {
        navigate(`/simstudio/${mostRecentProject.id}`, { replace: true });
          ...
          <ThemePanel translucent className="flex flex-col h-full overflow-hidden relative z-10 p-0">
            <div className="absolute top-4 left-4 z-50 flex gap-2">
                <button 
                  ...
                  className={`px-4 py-2 text-xs font-bold uppercase tracking-widest flex items-center gap-2 rounded-lg transition-all shadow-lg backdrop-blur-md ${mapType === 'satellite' ? 'bg-[#00ffcc] text-black' : 'bg-black/60 text-zinc-300 border border-zinc-700 hover:bg-zinc-800'}`}
                >
                  <Globe className="w-4 h-4" /> Earth Engine
                </button>
                <button 
                  onClick={() => setMapType('roadmap')} 
                  className={`px-4 py-2 text-xs font-bold uppercase tracking-widest flex items-center gap-2 rounded-lg transition-all shadow-lg backdrop-blur-md ${mapType === 'roadmap' ? 'bg-[#00ffcc] text-black' : 'bg-black/60 text-zinc-300 border border-zinc-700 hover:bg-zinc-800'}`}
                >
                  <MapIcon className="w-4 h-4" /> Vector Maps
                </button>
            </div>
            {isLoaded ? (
                <GoogleMap
          ...
          <ThemePanel translucent className="h-full overflow-hidden relative z-10">
             <div className="flex flex-col h-full overflow-hidden">
                <div className="px-4 py-3 border-b border-zinc-800 shrink-0 bg-transparent">
                    <h2 className="text-subheading font-normal text-white uppercase tracking-tighter">SIMULATION HUB</h2>
                </div>
