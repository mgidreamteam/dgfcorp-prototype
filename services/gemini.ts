...
export const getAnswerFromSpec = async (question: string, specs: HardwareSpec): Promise<string> => {
    const model = "gemini-3-flash-preview";
    const answerPrompt = `
    You are HELO, an expert hardware design assistant.
...
export const generateHardwareSpecs = async (prompt: string, previousSpec: HardwareSpec | null, projectName: string): Promise<HardwareSpec> => {
  ...
  if (previousSpec) {
    generationPrompt = `You are HELO, an agentic hardware designer. You previously generated the following hardware specification:
    ...
    `;
     ...
     generationPrompt = `You are HELO, an agentic hardware designer. Design a hardware product based on this description: "${prompt}". 
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          assumedParameters: {
            type: Type.ARRAY,
            description: "List of critical parameters that were not specified in the prompt and were assumed by HELO.",
            items: {
                type: Type.OBJECT,
                properties: {
