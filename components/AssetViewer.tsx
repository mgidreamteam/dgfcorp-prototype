...
import { DesignProject, DesignStatus, CircuitComponent } from '../types';
...
const QuadViewPanel: React.FC<{
...
}> = React.memo(({ title, icon: Icon, assetUrl, isLoading, isApplicable, productName, viewType, circuitComponents, onViewFullScreen, onDownloadImage }) => {
    ...
    const renderContent = () => {
        ...
        if (assetUrl) {
            const isCircuitWithComponents = viewType === 'circuit' && circuitComponents && circuitComponents.length > 0;
            return (
                 <>
                    <img 
                        src={assetUrl} 
                        alt={`${productName} - ${title}`} 
                        className={`w-full h-full object-contain transition-transform duration-700 group-hover:scale-105 ${isCircuitWithComponents ? 'absolute top-0 left-0 h-2/3' : ''}`}
                    />
                    {isCircuitWithComponents && <CircuitComponentTable components={circuitComponents} />}
...
const AssetViewer: React.FC<AssetViewerProps> = ({ assetUrls, status, productName, hasElectronics, circuitComponents }) => {
  ...
  const handleDownload = (imageUrl: string, productName: string, viewType: ViewType) => {
    ...
    const filename = `${productName.replace(/ /g, '_')}-${viewType}-${timestamp}`;

    // If it's an SVG, download it directly as a .svg file to preserve vector quality
    ...
    // Existing logic for raster images (PNG/JPG) with watermarking
    ...
    image.onload = () => {
        ...
        const watermarkText = `DREAMed by: ${username || 'User'}`;
