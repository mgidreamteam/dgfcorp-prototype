      ...
      {project.status === DesignStatus.ERROR && (
        <div className="mt-8 bg-red-900/20 border border-red-500/30 rounded-xl p-4 flex items-center justify-between no-print">
          <div className="text-red-300">
            ...
            <p className="text-sm">Step: <span className="font-kido">{project.failedStep}</span></p>
          </div>
        ...
        <>
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 backdrop-blur-sm animate-fade-in">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white mb-1">{project.specs.productName}</h2>
                <p className="text-white/80 font-medium text-lg">{project.specs.tagline}</p>
              </div>
              <div className="bg-white/10 text-zinc-300 px-3 py-1 rounded-full text-xs font-kido border border-white/20">v1.0-draft</div>
            </div>
            <p className="text-zinc-300 mt-4 leading-relaxed">{project.specs.description}</p>
            <div className="mt-6 pt-6 border-t border-zinc-800">
              <h4 className="text-sm font-bold text-zinc-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                <Factory className="w-4 h-4" />
                Manufacturing Process
              </h4>
              <p className="text-zinc-300">{project.specs.manufacturingProcess}</p>
            </div>
          </div>
          <SpecViewer 
            specs={project.specs} 
